...
function updateExamModeInterface(examMode) {
    const toggle = document.getElementById('exam-mode-toggle');
    const normalOption = document.getElementById('mode-normal-option');
    const examOption = document.getElementById('mode-exam-option');
    ...
    if (examMode) {
        ...
        examOption.classList.add('active');
        modeInfo.innerHTML = 'Mode actuel : <strong>Examen</strong> 📝';
        container.classList.add('exam-mode-active');
    } else {
        normalOption.classList.add('active');
        examOption.classList.remove('active');
        modeInfo.innerHTML = 'Mode actuel : <strong>Normal</strong> 💬';
        container.classList.remove('exam-mode-active');
    }
}

async function handleExamModeToggle() {
    const toggle = document.getElementById('exam-mode-toggle');
    const examMode = toggle.checked;

    try {
        await chrome.storage.local.set({ examMode });
        updateExamModeInterface(examMode);

        const modeText = examMode ? 'Examen' : 'Normal';
        showStatus(`✅ Mode ${modeText} activé !`, 'success');
    } catch (error) {
...
document.getElementById('open-benchmark-page-btn').addEventListener('click', () => {
    chrome.tabs.create({
        url: chrome.runtime.getURL('test/benchmark.html')
    });
});
